...
# 導入現有的 MCP Shouji 模組
try:
    from mcp_shouji.debug import server_debug_log as debug_log
    from mcp_shouji.web.main import WebUIManager, get_web_ui_manager
except ImportError as e:
    print(f"无法导入 MCP Shouji 模块: {e}")
    sys.exit(1)
...
class DesktopApp:
    ...
    async def launch_tauri_app(self, server_url: str):
        ...
        try:
            from mcp_shouji.desktop_release import __file__ as desktop_init
            ...
            # 定義平台到二進制文件的映射
            if system == "windows":
                tauri_exe = desktop_dir / "mcp-shouji-desktop.exe"
            elif system == "darwin":  # macOS
                # 檢測 Apple Silicon 或 Intel
                if machine in ["arm64", "aarch64"]:
                    tauri_exe = (
                        desktop_dir / "mcp-shouji-desktop-macos-arm64"
                    )
                else:
                    tauri_exe = (
                        desktop_dir / "mcp-shouji-desktop-macos-intel"
                    )
            elif system == "linux":
                tauri_exe = desktop_dir / "mcp-shouji-desktop-linux"
            else:
                # 回退到通用名稱
                tauri_exe = desktop_dir / "mcp-shouji-desktop"
            ...
            if tauri_exe.exists():
                ...
                # 嘗試回退選項
                fallback_files = [
                    desktop_dir / "mcp-shouji-desktop.exe",
                    desktop_dir / "mcp-shouji-desktop-macos-intel",
                    desktop_dir / "mcp-shouji-desktop-macos-arm64",
                    desktop_dir / "mcp-shouji-desktop-linux",
                    desktop_dir / "mcp-shouji-desktop",
                ]
