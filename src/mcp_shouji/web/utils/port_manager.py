...
class PortManager:
    ...
    def kill_process_on_port(port: int, force: bool = False) -> bool:
        ...
        try:
            ...
            # 檢查是否是自己的進程（避免誤殺）
            if "mcp-shouji" in process_info["cmdline"].lower():
                debug_log("检测到 MCP Shouji 相关进程，尝试优雅终止")
    ...
    def _should_cleanup_process(process_info: dict[str, Any]) -> bool:
        ...
        Returns:
            bool: 是否應該清理該進程
        """
        # 檢查是否是 MCP Shouji 相關進程
        cmdline = process_info.get("cmdline", "").lower()
        process_name = process_info.get("name", "").lower()

        # 如果是自己的進程，允許清理
        if any(
            keyword in cmdline for keyword in ["mcp-shouji", "mcp_shouji"]
        ):
            return True
