...
MCP Shouji 支援多種部署環境，具備智能環境檢測和自適應配置能力。
...
```bash
# 直接運行
uvx mcp-shouji@latest web

# 指定版本
uvx mcp-shouji@2.4.3 web
```

#### 2. 使用 pip
```bash
# 安裝
pip install mcp-shouji
...
```bash
# 克隆倉庫
git clone https://github.com/f/mcp-shouji.git
cd mcp-shouji
...
# 運行
uv run python -m mcp_shouji web
```
...
```bash
# 在遠程服務器運行
mcp-shouji web --host 0.0.0.0 --port 8000
```
...
RUN pip install mcp-shouji

EXPOSE 8000

CMD ["mcp-shouji", "web", "--host", "0.0.0.0", "--port", "8000"]
```

```bash
# 構建和運行
docker build -t mcp-shouji .
docker run -p 8000:8000 mcp-shouji
```
...
# 解決方案：使用不同埠
mcp-shouji web --port 8001
```

2. **瀏覽器無法開啟**
```bash
# 手動開啟瀏覽器
mcp-shouji web --no-browser
...
```bash
# 啟用詳細日誌
mcp-shouji web --debug

# 查看詳細錯誤信息
export PYTHONPATH=.
python -m mcp_shouji.debug
```
...
# 或使用 pip 升級
pip install --upgrade mcp-shouji==2.4.3
```
