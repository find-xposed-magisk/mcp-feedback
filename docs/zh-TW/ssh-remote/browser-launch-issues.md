...
在 SSH Remote 環境（如 Cursor SSH Remote、VS Code Remote SSH 、WSL 等）中使用 MCP Shouji 時，可能會遇到以下問題：
...
MCP Shouji 預設使用端口 **8765**，您也可以自定義端口：
...
### ⚠️ 不要手動啟動
**請勿**手動執行 `uvx mcp-shouji test --web` 等指令，這樣無法與 MCP 系統整合。
