...
在 SSH Remote 环境（如 Cursor SSH Remote、VS Code Remote SSH 等）中使用 MCP Shouji 时，可能会遇到以下问题：
...
MCP Shouji 默认使用端口 **8765**，您也可以自定义端口：
...
### ⚠️ 不要手动启动
**请勿**手动执行 `uvx mcp-shouji test --web` 等指令，这样无法与 MCP 系统整合。
