# Makefile for mcp-shouji development
# 適用於 mcp-shouji 專案開發
...
test-cov: ## Run tests with coverage
	uv run pytest --cov=src/mcp_shouji --cov-report=html --cov-report=term
...
test-func: ## Run functional tests (standard)
	uv run python -m mcp_shouji test

test-web: ## Run Web UI tests (continuous)
	uvx --no-cache --with-editable . mcp-shouji test --web

test-desktop-func: ## Run desktop application functional tests
	uvx --no-cache --with-editable . mcp-shouji test --desktop
...
test-desktop: build-desktop ## Test desktop application
	...
	uv run python -m mcp_shouji test --desktop
